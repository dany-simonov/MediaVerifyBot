...
export function FeatureCard({ icon, title, description, badge }: FeatureCardProps) {
  return (
    <div className="relative h-full min-h-[220px] bg-mv-surface border border-mv-border rounded-xl p-6 hover:border-mv-accent/50 hover:-translate-y-1 transition-all duration-300">
      {badge && (
      ...
      <p className="text-sm text-mv-text-secondary leading-relaxed min-h-[72px]">{description}</p>
    </div>
  );
}
